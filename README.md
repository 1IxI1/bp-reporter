...
Telegram JSONL export распознается автоматически:

```bash
uv run python convert_history.py bp.jsonl history-output \
  --timezone Europe/Minsk
```

Для каждого сообщения parser берет дату из первой строки вида
`21 марта 09:51`. Год выбирается относительно Telegram timestamp. Опечатка,
невозможная дата или расхождение более 14 дней считаются ошибочной датой, и тогда
используется локальное время поля `date` самого сообщения. Каждое исходное
`#N: SYS/DIA` или `SYS на DIA` становится отдельной строкой; строки со средним
`#:` игнорируются. Так как пульса в экспорте нет, `Heart rate` остается пустым.
Измерения одной серии разводятся на одну секунду, чтобы одинаковые значения не
схлопнулись как дубликаты.
...
В `conversion-report.json` отдельно перечисляются сообщения, где использован
fallback timestamp, а также пропущенные и некорректные строки.
